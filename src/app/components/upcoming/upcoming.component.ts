...
export class UpcomingComponent {
  tasks: TodoTask[] = [];
  constructor(private dialog: MatDialog, public tasksService: TasksService, private dateService : DateService) {}

  ngOnInit(): void {
    if (dev) {
      this.tasksService.taskList = this.tasksService.getMockTasks(5);
    } else {
      console.log('ngOnInit - upcoming');
      this.tasksService.getTasks(0).subscribe((tasks: any) => {
        this.tasksService.taskList = tasks.filter((task: TodoTask) => {
          return this.dateService.compareDates(this.dateService.converDotNEtDateToTypeScriptDate(task.dueDate.toString()), new Date()) === 1;
        });
